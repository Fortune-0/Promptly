window.onload = () => {
    ...
    async () => {
        const response = await fetch('http://127.0.0.1:5000/api/allReminders');
        const tasks = await response.json();
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        console.log(tasks); // Check for response to determine the var name for all the var in taskParentContainer.innerHTML
